...
export default function ListingsPage({ searchParams }: { searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  const filtered = listings.filter((listing) => {
    ...
    const matchesLocation = !location || `${listing.city} ${listing.region}`.toLowerCase().includes(location.toLowerCase());
    return matchesCategory && matchesCondition && matchesLocation;
  });
            ...
            <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
              <div>
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <Link href="/signup" className="inline-flex items-center rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-soft">
                    Start selling
                  </Link>
                  <button className="inline-flex items-center rounded-xl bg-muted px-5 py-3 text-sm font-semibold text-foreground">
              ...
              <div className="rounded-[1.75rem] bg-gradient-to-br from-secondary to-secondary/80 p-6 text-secondary-foreground shadow-soft">
                ...
                <Link href="/listings/vintage-cider-press" className="mt-6 inline-flex items-center rounded-xl bg-white px-4 py-2 text-sm font-semibold text-secondary">
