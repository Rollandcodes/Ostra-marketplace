...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-black/5 bg-white/80 backdrop-blur-2xl">
      <div className="section-shell flex h-16 items-center justify-between gap-3 md:gap-4">
        <div className="flex min-w-0 items-center gap-5 xl:gap-8">
          <Link href="/" className="max-w-[68vw] truncate font-display text-lg font-extrabold tracking-tight text-primary sm:max-w-none sm:text-xl">
            Ostra Marketplace
          </Link>
        ...
        <div className="hidden items-center gap-2 xl:flex">
          <form onSubmit={submitSearch} className="flex items-center gap-2 rounded-full bg-muted px-3 py-2 text-sm text-foreground/70">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} className="w-40 bg-transparent outline-none placeholder:text-foreground/40 2xl:w-48" placeholder="Search marketplace..." />
          </form>
          <label className="inline-flex items-center gap-2 rounded-full bg-muted px-3 py-2 text-sm font-medium text-foreground/80">
            ...
            <select
              ...
              className="max-w-28 bg-transparent outline-none 2xl:max-w-none"
            >
              {regions.map((region) => (
        ...
        <button onClick={() => setMobileOpen((value) => !value)} className="inline-flex items-center rounded-full bg-muted p-2 xl:hidden" aria-label="Menu" type="button">
          <Menu className="h-5 w-5" />
        </button>
      </div>
      {mobileOpen ? (
        <div className="section-shell space-y-4 border-t border-black/5 py-4 xl:hidden">
          ...
          <nav className="grid gap-2">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="rounded-xl bg-muted px-4 py-2 text-sm font-medium leading-5" onClick={() => setMobileOpen(false)}>
                {link.label ?? t(link.labelKey as Parameters<typeof t>[0], locale)}
              </Link>
            ))}
          </nav>
          <Show when="signed-out">
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <SignInButton mode="modal">
                <button type="button" className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground">
                  <CircleUserRound className="h-4 w-4" />
                  Login
                </button>
              </SignInButton>
              <SignUpButton mode="modal">
                <button type="button" className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-secondary px-4 py-2.5 text-sm font-semibold text-secondary-foreground">
                  Sign up
                </button>
              </SignUpButton>
            </div>
